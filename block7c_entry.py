...
def _resolve_missing_ratio(b6a: dict) -> float:
    data_gate = b6a.get("data_gate", {}) if isinstance(b6a, dict) else {}
    startup = b6a.get("startup_validation", {}) if isinstance(b6a, dict) else {}

    candidates = [
        data_gate.get("missing_ratio_pct"),
        startup.get("missing_ratio_pct"),
    ]
...
def main():
    ...
    b6a = _read_json(".state/block6a_data_adapters.json", {"data_gate": {}, "startup_validation": {}})
    ...
    wf_avg_return = _safe_float(b5b.get("avg_test_return_pct", 0.0), 0.0)
    mc_negative_run_pct = _safe_float(b5b.get("risk_of_negative_run_pct", 100.0), 100.0)
    missing_ratio_pct = _resolve_missing_ratio(b6a)
