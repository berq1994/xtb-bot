...
def main():
    ...
    # režim z env (run / learn / backfill)
    run_mode = (os.getenv("RUN_MODE") or "run").strip().lower()

    # ✅ časy – priorita: ENV -> config.yml -> default
    ...
    # --- 07:30 ranní radar (Telegram + Email 1x denně) ---
    if now_hm == premarket_time and not st.already_sent("premarket", today):
        ...
        # email 1× denně – z ranního reportu
        maybe_send_email_report(cfg, snapshot, now, tag="premarket")

        st.mark_sent("premarket", today)

    # --- 20:00 večerní radar (Telegram, email ne – dle tvého pravidla max 1× denně) ---
    ...
    # --- Alerty každých 15 minut v okně ---
    if in_window(now_hm, alert_start, alert_end):
        alerts = run_alerts_snapshot(cfg, now, st)  # ✅ POZIČNĚ, žádné state=
        if alerts:
