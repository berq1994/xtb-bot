...
class State:
    ...
    # ---- daily alert cap ----
    def _get_alert_counter(self) -> Dict[str, Any]:
        cur = self.sent.get("alerts_counter")
        if isinstance(cur, dict):
            return cur
        return {"day": "", "count": 0}

    def remaining_alerts(self, day: str, cap: int) -> int:
        cap = int(cap or 0)
        if cap <= 0:
            return 999999
        cur = self._get_alert_counter()
        if str(cur.get("day")) != str(day):
            cur = {"day": str(day), "count": 0}
            self.sent["alerts_counter"] = cur
            return cap
        try:
    ...
    def increment_alerts(self, day: str, n: int = 1) -> None:
        cur = self._get_alert_counter()
        if str(cur.get("day")) != str(day):
            cur = {"day": str(day), "count": 0}
        try:
            cur["count"] = int(cur.get("count") or 0) + int(n or 0)
        except Exception:
