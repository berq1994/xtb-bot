...
def build_execution_guard(
    ...
    if risk_overlay.get("macro_lock"):
        ...
        blocked_reasons.append("Macro lock je aktivní.")
    elif risk_overlay.get("earnings_lock") and recommended_mode in {"DEFENSIVE", "CAUTIOUS"}:
        allow_new_risk = False
        guard_status = "BLOCK_NEW_RISK"
        blocked_reasons.append("Earnings lock je aktivní pro nové risk vstupy.")
    elif risk_posture == "LOCK NEW RISK":
        allow_new_risk = False
        guard_status = "BLOCK_NEW_RISK"
    ...
    elif recommended_mode == "SELECTIVE":
        allow_new_risk = True
    ...
    if performance_summary.get("overall_hit_rate", 0.0) < 0.45 and performance_summary.get("scored_records", 0) >= 5:
