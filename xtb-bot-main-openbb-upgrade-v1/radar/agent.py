...
class RadarAgent:
    ...
    def show_portfolio(self) -> AgentResponse:
        """Pretty portfolio snapshot (Telegram-friendly)."""
        ...
        def _bar(pct: float | None) -> str:
            if pct is None:
                return ""
            v = max(-10.0, min(10.0, float(pct)))
            n = int(round(abs(v) / 2.0))  # 0..5
        ...
        for r in self.cfg.portfolio:
            ...
            qty = _to_f(r.get("qty") or r.get("shares") or r.get("amount") or 0.0) or 0.0
            avg = _to_f(r.get("avg") or r.get("avg_price") or r.get("buy_price") or r.get("price"))
            px = get_price_data(t, self.cfg.model_dump())
            last = px.get("last")
            chg = px.get("change_pct")
            pl = None
            pl_pct = None
            if last is not None and avg is not None and qty:
                pl = (last - avg) * qty
                cost = avg * qty
                total_pl += pl
                total_cost += cost
            ...
            rows.append({
                ...
                "1D%": f"{chg:+.2f}%" if isinstance(chg, (int, float)) else "n/a",
                "P/L": f"{pl:+.2f}" if isinstance(pl, (int, float)) else "n/a",
                "P/L%": f"{pl_pct:+.2f}%" if isinstance(pl_pct, (int, float)) else "n/a",
                "Bar": _bar(chg),
            })
            if isinstance(chg, (int, float)):
                movers.append((t, float(chg)))
        ...
        lines = []
        lines.append("## Portfolio + top pohyby")
        if total_pl_pct is not None:
            icon = "🟢" if total_pl >= 0 else "🔴"
            lines.append(f"{icon} Celkem P/L: **{total_pl:+.2f}** ({total_pl_pct:+.2f}%)")
        lines.append("")
        lines.append("| Ticker | Last | 1D | P/L | P/L% | |")
        lines.append("|---|---:|---:|---:|---:|---|")
        for row in rows:
            lines.append(f"| {row['Ticker']} | {row['Last']} | {row['1D%']} | {row['P/L']} | {row['P/L%']} | {row['Bar']} |")

        if movers_sorted:
            lines.append("")
            lines.append("### Top pohyby (1D)")
            for t, chg in movers_sorted:
                lines.append(f"- {('🟩' if chg>=0 else '🟥')} **{t}** {chg:+.2f}%")

        lines.append("")
        lines.append("_Pozn.: Data jsou orientační, bez záruky; nejde o investiční doporučení._")
