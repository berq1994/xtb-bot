...
def _clean_long_score(signal: dict) -> float:
    ...
    if ta_setup in {'pullback', 'breakout', 'range'}:
        score += 0.08
    elif ta_setup not in {'', 'none', 'unknown', 'breakdown'}:
        score += 0.04
...
def build_learning_summary(limit: int = 80) -> dict:
    ...
    for outcome in outcome_rows:
        ...
        signal = _merge_missing_signal_context(signal, latest_research_index, fundamentals_index)
    ...
    for row in resolved_all:
        ...
        if not _learnable_row(signal, outcome):
            ...
            features = signal.get('features', {}) if isinstance(signal, dict) else {}
            reason = []
            if str(features.get('evidence_grade', '?')).upper() in {'D', '?'} and _clean_long_score(signal) < 1.0:
                reason.append('weak_evidence')
            if float(features.get('data_quality_score', signal.get('data_quality_score', 0.0)) or 0.0) < 0.5:
                reason.append('low_data_quality')
            ta_decision = str(features.get('buy_decision') or signal.get('buy_decision') or '').lower()
            ta_setup = str(features.get('technical_setup') or signal.get('technical_setup') or '').lower()
            if ta_decision == 'avoid' or ta_setup == 'breakdown':
                reason.append('ta_block')
            if not reason:
                reason.append('gate')
...
def run_learning_review(limit: int = 80) -> str:
    ...
    for key, value in summary.get('rejected_mix', {}).items():
        lines.append(f"- {key}: {value}")
    lines.append("Důvody odmítnutí:")
    for key, value in summary.get('rejected_reasons', {}).items():
        lines.append(f"- {key}: {value}")
    lines.append("Štítky:")
